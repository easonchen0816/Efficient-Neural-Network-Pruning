...
class GatedBatchNorm2d(Meltable):
    def __init__(self, bn, minimal_ratio = 0.1):
        ...
        #self.register_buffer('g_ones', torch.ones(1, self.channel_size, 1, 1).to(self.device))
...
class IterRecoverFramework():
    def __init__(self, pack, masks, flops_eta=0.0, minium_filter=10):
        self.pack = pack
        self.masks = masks
        self.status = {}
        self.logs = []
    ...
    def tock(self, lr_min=0.0005, lr_max=0.01, tock_epoch = 20, mute=False, acc_step=1):
        logs = []
        epoch = 0
        T = tock_epoch
        best_acc = 0
        ...
        for i in range(T):
            ...
            epoch += 1
            if not mute:
                print('Finetune - %d\t Test Loss: %.4f\t Test Acc: %.2f\t Final LR: %.5f' % (i, info['test_loss'], info['acc@1'], info['LR']))
        ...
        print('Best\t Test Loss: %.4f\t Test Acc: %.2f\t Final LR: %.5f' % (info['test_loss'], info['acc@1'], info['LR']))
        return logs
    ...
    def prune(self, num, tick=False, lr=0.01, test=True):
        ...
        for g in self.masks:
            if g.group_id in self.status:
                ...
                self.status[g.group_id]['score'] += g.get_score(self.flops_eta / self.eta_scale_factor).cpu().data.numpy()
