...
class NormalTrainer():
    ...
    def test(self, pack, topk=(1,)):
        ...
        return info
    def train_ir(self, pack, update=True, iter_hook=None, mute=False, acc_step=1, max_iter=123):
        pack.net.train()
        loss_acc, correct_acc, total = 0.0, 0.0, 0.0
        begin = time()
        # max_iter = len(pack.train_loader)
        pack.optimizer.zero_grad()
        with tqdm(total=max_iter, disable=mute) as pbar:
            loader_i = iter(pack.train_loader_i)
            for cur_iter, (data, label) in enumerate(pack.train_loader):
                if iter_hook is not None:
                    ...
                    if signal == FINISH_SIGNAL:
                        break
                data_i, label_i = loader_i.next()
                if self.use_cuda:
                    data_i, label_i = data_i.cuda(), label_i.cuda()
                data_i = Variable(data_i, requires_grad=False)
                label_i = Variable(label_i)
                pack.net.module.mod = 'Imagenet'
                logits_i = pack.net(data_i)
                loss_i = pack.criterion(logits_i, label_i)
                loss_i = loss_i / acc_step
                loss_i.backward()
                if update:
                    pack.optimizer.step()
                pack.optimizer.zero_grad()
                pack.net.module.mod = 'Caltech'
                '''if cur_iter == 0:
